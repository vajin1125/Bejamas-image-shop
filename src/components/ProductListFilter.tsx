...
function ProductListFilter() {
	...
	const updateCategoryFilter = (filterArr:any) => {
		let filterCategoryList = products
		let filteredArray:any = []
		filterArr.map((filter:any) => {
			const response = filterCategoryList.filter((product:any) => product.category === filter)
	...
	const updatePriceFilter = (filterString:any, prevFilteredProducts:any) => {
		if (filterString.length > 0) {
			...
			let priceArrFilter = prevFilteredProducts.filter((product:any) => product.price >= lowerRange && product.price <= upperRange)
			if (upperRange === 0) priceArrFilter = prevFilteredProducts.filter((product:any) => product.price >= lowerRange)
